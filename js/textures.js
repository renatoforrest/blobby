...
const BTN_W = 168, BTN_H = 36;
const BTN_SM_W = 132, BTN_SM_H = 29;
...
const skyTex = makeTex(VW, 228, (ctx) => {
  const g = ctx.createLinearGradient(0, 0, 0, 228);
  g.addColorStop(0.00, '#0a2f66');
  g.addColorStop(0.40, '#2a6fb8');
  g.addColorStop(1.00, '#a5d2ec');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, VW, 228);
});

const cloudTex = makeTex(96, 48, (ctx) => {
  ctx.scale(0.6, 0.6);
  ctx.beginPath();
  ...
  ctx.fillStyle = 'rgba(210,228,246,0.85)';
  ctx.fill();
});

const oceanTex = makeTex(VW, 42, (ctx) => {
...
const sandTex = makeTex(VW, 108, (ctx) => {
  ...
  for (let i = 0; i < 260; i++) {
    ctx.fillRect((i * 97) % VW, (i * 53) % 108, 1, 1);
  }
});

const cliffTex = makeTex(108, 72, (ctx) => {
  ctx.scale(0.6, 0.6);
...
const rockTex = makeTex(30, 18, (ctx) => {
  ctx.scale(0.6, 0.6);
  ctx.fillStyle = '#5c574c';
  ctx.beginPath();
...
function makePalmTex(lean) {
  return makeTex(156, 216, (ctx) => {
    ...
    ctx.beginPath();
    ctx.ellipse(baseX + 24, 342, 44, 9, 0, 0, Math.PI * 2);
    ctx.fill();
...
function makeFallbackBlobTex(body, dark, eye) {
  const S = BLOB_R * 2;
  return makeTex(S, S, (ctx) => {
    ctx.scale(S / 100, S / 100);

    /* Dark silhouette (slightly larger) */
    ctx.beginPath();
    ctx.moveTo(8, 90);
    ctx.bezierCurveTo(-2, 68, 10, 20, 50, 4);
    ctx.bezierCurveTo(90, 20, 102, 68, 92, 90);
    ctx.quadraticCurveTo(50, 98, 8, 90);
    ctx.closePath();
    ctx.fillStyle = dark;
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(14, 87);
    ctx.bezierCurveTo(5, 66, 18, 24, 50, 11);
    ctx.bezierCurveTo(82, 24, 95, 66, 86, 87);
    ctx.quadraticCurveTo(50, 93, 14, 87);
    ctx.closePath();
    ctx.fillStyle = body;
    ctx.fill();
    ...
    ctx.fillStyle = eye;
    ctx.beginPath(); ctx.arc(41, 32, 3.2, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(59, 32, 3.2, 0, Math.PI * 2); ctx.fill();
  });
}
...
const netTex = makeTex(NET_VW, GROUND_Y - NET_TOP, (ctx, w, h) => {
  ...
  ctx.fillStyle = 'rgba(180,205,230,0.9)';
  ctx.fillRect(6, 0, 1, h);
...
const rowBgTex = makeTex(360, 24, (ctx, w, h) => {
  const r = 5;
  ctx.beginPath();
  ctx.moveTo(r, 0);
  ctx.arcTo(w, 0, w, h, r); ctx.arcTo(w, h, 0, h, r);
...
async function loadBlobAndBallTextures() {
  const results = await Promise.all([
    loadOne(SPRITE_URLS.blobRed, 'blobRed',
      () => makeFallbackBlobTex('#e62e2e', '#7a0f0f', '#000000')),
    loadOne(SPRITE_URLS.blobYellow, 'blobYellow',
      () => makeFallbackBlobTex('#2860c8', '#0f2a70', '#000000')),
